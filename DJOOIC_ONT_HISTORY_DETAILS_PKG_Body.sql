--------------------------------------------------------
--  DDL for Package Body DJOOIC_ONT_HISTORY_DETAILS_PKG
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."DJOOIC_ONT_HISTORY_DETAILS_PKG" AS
  -- XX_ORDER_SEARCH procedure
    ...
    -- Initialize the dynamic SQL query
        v_query := 'SELECT distinct CASE WHEN oha.order_source_id = 1144 THEN oha.ORIG_SYS_DOCUMENT_REF ELSE NULL END ,oha.header_id,oha.order_number,oha.flow_status_code,oha.ordered_date,oha.booked_date,oha.cust_po_number '
        ;
        v_query_count := 'SELECT count(distinct oha.order_number) ';
        ...
        if p_tracking_number is not null then
             v_query := v_query || ', apps.oe_order_lines_all ola, apps.wsh_delivery_details     wdd ';
             v_query_count := v_query_count || ', apps.oe_order_lines_all ola, apps.wsh_delivery_details     wdd ';
        end if;
        
        if p_part_number is not null then
             v_query := v_query || ', apps.oe_order_lines_all ola ';
             v_query_count := v_query_count || ', apps.oe_order_lines_all ola ';
        end if;
        ...
        if p_tracking_number is not null then
             v_query := v_query || 'AND ola.header_id = oha.header_id and wdd.source_header_id = oha.header_id AND wdd.source_line_id = ola.line_id ';
             v_query_count := v_query_count || 'AND ola.header_id = oha.header_id and wdd.source_header_id = oha.header_id AND wdd.source_line_id = ola.line_id ';
        end if;
        
        if p_part_number is not null then
             v_query := v_query || 'AND ola.header_id = oha.header_id ';
             v_query_count := v_query_count || ' AND ola.header_id = oha.header_id ';
        end if;
...
END IF;
        ...
        IF
            p_part_number IS NOT NULL
        THEN
        IF p_operator_code = 'IS' THEN
         v_query := v_query || ' AND UPPER(ola.ordered_item) = '''|| UPPER(p_part_number) ||'''';
         v_query_count := v_query_count || ' AND UPPER(ola.ordered_item) = '''|| UPPER(p_part_number) ||'''';

       ELSIF p_operator_code = 'CONTAIN' THEN
         v_query := v_query || ' AND UPPER(ola.ordered_item) like ''%'|| UPPER(p_part_number) ||'%''';
         v_query_count := v_query_count || ' AND UPPER(ola.ordered_item) like ''%'|| UPPER(p_part_number) ||'%''';
...
END IF;
        ...
        IF
            p_tracking_number IS NOT NULL
        THEN
        IF p_operator_code = 'IS' THEN
         v_query := v_query || ' AND UPPER(wdd.tracking_number) = '''|| UPPER(p_tracking_number) ||'''';
         v_query_count := v_query_count || ' AND UPPER(wdd.tracking_number) = '''|| UPPER(p_tracking_number) ||'''';

       ELSIF p_operator_code = 'CONTAIN' THEN
         v_query := v_query || ' AND UPPER(wdd.tracking_number) like ''%'|| UPPER(p_tracking_number) ||'%''';
         v_query_count := v_query_count || ' AND UPPER(wdd.tracking_number) like ''%'|| UPPER(p_tracking_number) ||'%''';
...
WHERE     1 = 1
    ...
    l_line_details l_line_details_table;
        ...
        cursor c_order_line(cp_header_id NUMBER)
        IS
        SELECT 
 ...
 WHERE     oha.header_id = ola.header_id
       ...
       and uom.language = 'E'
       order by ola.line_number;
       --AND oha.order_number = '3973563'
       
    CURSOR c_track_details(cp_header_id NUMBER, cp_line_id NUMBER, cp_line_number NUMBER)
    IS
    SELECT
        cp_line_number       LINE_NUMBER,
    ...
    -- validate input parameters
        ...
        OPEN c_order_line(l_header_id);

        
        FETCH c_order_line BULK COLLECT INTO l_line_details;
        CLOSE c_order_line;
        ...
        FOR i IN 1..l_line_details.count LOOP
            ...
            l_result.EXTEND;
            l_result(i).line_number := l_line_details(i).line_number;
            ...
            l_result(i).scheduled_ship_date := l_line_details(i).scheduled_ship_date;
            
            dbms_output.put_line('line number'||l_line_details(i).line_id);
            dbms_output.put_line('After assignment');
           OPEN c_track_details(l_result(i).header_id, l_result(i).line_id, l_result(i).line_number);
...
FROM
 ...
 WHERE
addr.interfaced_flag = ''I''  order by (account_id) offset nvl(('
           || p_counter
           || '-1),0) * '
           || p_limit
           || ' rows FETCH NEXT '
...
OPEN v_refcur FOR v_query;
    ...
    CURSOR c_parent_org_details(cp_email_address VARCHAR2)
    IS
    SELECT
    account_id,
        role
...
OPEN v_refcur FOR v_query;
    ...
    FOR i in 1..l_profile_details.COUNT LOOP
        dbms_output.put_line(l_profile_details(i).account_id);
        l_result.EXTEND;
